    ...
    if len(data.split()) < 2:
        mm = [
            InlineQueryResultArticle(
                ...
                thumb_url="https://files.catbox.moe/x9ozjy.jpg",
...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 7824722052]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 Whisper",
            description=f"@NobitaMusicsRobot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**📍Usage:**\n\n@NobitaMusicsRobot (Target Username or ID) (Your Message).\n\n**Example:**\n@NobitaMusicsRobot @username I Wanna Phuck You"),
            thumb_url="https://files.catbox.moe/x9ozjy.jpg",
            reply_markup=switch_btn
        )
    ]
