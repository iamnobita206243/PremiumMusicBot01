...
mediadeletedb = mongodb.mediadelete
mediadeletetimedb = mongodb.mediadeletetime
mediadeletetypedb = mongodb.mediadeletetype
...
# Media Delete Protection Database Methods

async def is_media_delete_on(chat_id: int) -> bool:
    mode = media_delete.get(chat_id)
    if mode is None:
        mode = await mediadeletedb.find_one({"chat_id": chat_id})
        if not mode:
            media_delete[chat_id] = False
            return False
        media_delete[chat_id] = mode["status"]
        return mode["status"]
    return mode

async def media_delete_on(chat_id: int):
    media_delete[chat_id] = True
    await mediadeletedb.update_one(
        {"chat_id": chat_id}, {"$set": {"status": True}}, upsert=True
    )

async def media_delete_off(chat_id: int):
    media_delete[chat_id] = False
    await mediadeletedb.update_one(
        {"chat_id": chat_id}, {"$set": {"status": False}}, upsert=True
    )

async def get_media_delete_time(chat_id: int) -> int:
    time = media_delete_time.get(chat_id)
    if not time:
        time_db = await mediadeletetimedb.find_one({"chat_id": chat_id})
        if not time_db:
            media_delete_time[chat_id] = 3  # default 3 mins
            return 3
        media_delete_time[chat_id] = time_db["time"]
        return time_db["time"]
    return time

async def set_media_delete_time(chat_id: int, time: int):
    media_delete_time[chat_id] = time
    await mediadeletetimedb.update_one(
        {"chat_id": chat_id}, {"$set": {"time": time}}, upsert=True
    )

async def get_media_delete_type(chat_id: int) -> str:
    type_val = media_delete_type.get(chat_id)
    if not type_val:
        type_db = await mediadeletetypedb.find_one({"chat_id": chat_id})
        if not type_db:
            media_delete_type[chat_id] = "All"
            return "All"
        media_delete_type[chat_id] = type_db["type"]
        return type_db["type"]
    return type_val

async def set_media_delete_type(chat_id: int, type_val: str):
    media_delete_type[chat_id] = type_val
    await mediadeletetypedb.update_one(
        {"chat_id": chat_id}, {"$set": {"type": type_val}}, upsert=True
    )
